    ...
    return Column(
      children: [
        _buildModulationMeter(
          ...
          ((state['rotationXW'] as double).abs() / math.pi).clamp(0.0, 1.0),
          '${((state['rotationXW'] as double) / math.pi * 2).toStringAsFixed(2)} st',
          Colors.purple,
        ),
        _buildModulationMeter(
          'YW Rotation → Osc2 Freq',
          ((state['rotationYW'] as double).abs() / math.pi).clamp(0.0, 1.0),
          '${((state['rotationYW'] as double) / math.pi * 2).toStringAsFixed(2)} st',
          Colors.pink,
        ),
        _buildModulationMeter(
          'ZW Rotation → Filter Cutoff',
          ((state['rotationZW'] as double).abs() / math.pi).clamp(0.0, 1.0),
          '${((state['rotationZW'] as double) / math.pi * 40).toStringAsFixed(0)}%',
          Colors.blue,
        ),
        _buildModulationMeter(
          'Morph → Wavetable',
          state['morphParameter'] as double,
          '${((state['morphParameter'] as double) * 100).toStringAsFixed(0)}%',
          Colors.teal,
        ),
        _buildModulationMeter(
          'Projection → Reverb',
          (state['projectionDistance'] as double).clamp(0.0, 1.0),
          '${((state['projectionDistance'] as double) * 100).toStringAsFixed(0)}%',
          Colors.indigo,
        ),
        _buildModulationMeter(
          'Layer Depth → Delay',
          (state['layerSeparation'] as double).clamp(0.0, 1.0),
          '${((state['layerSeparation'] as double) * 500).toStringAsFixed(0)} ms',
          Colors.deepPurple,
        ),
      ],
    );
